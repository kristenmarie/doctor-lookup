...
export class DoctorLookup {
  ...
  .then(function(response){
    ...
    $.get(`https://api.betterdoctor.com/2016-03-01/doctors?query=${ search }&location=${ latitude }%2C${ longitude }%2C50&user_location=${ longitude }%2C${ latitude }&skip=0&limit=10&user_key=${ apiKey }`)
    .then(function(response){
 ...
 .fail(function(){
  ...
  .then(function(response){
    let latitude = response.results[0].geometry.location.lat;
    let longitude = response.results[0].geometry.location.lng;
    $.get(`https://api.betterdoctor.com/2016-03-01/doctors?name=${ doctor }&location=${ latitude }%2C${ longitude }%2C50&user_location=${ latitude }%2C${ longitude }&skip=0&limit=10&user_key=${ apiKey }`)
    .then(function(response){
      let results = response.data;
      if(results.length == 0) {
        $('#results').append(`<h2>No doctors matched your query.</h2>`);
